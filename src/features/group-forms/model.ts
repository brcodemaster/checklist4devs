...
const formSchema = z.object({
	...
	slug: z
		.string({ error: 'Required field' })
		.min(3, { error: 'Group slug must be at least 3 characters long' }),
	isPublic: z.boolean()
})

export type TForm = z.infer<typeof formSchema>

export const useGroupForms = (name?: string, slug?: string | null, isPublic?: boolean) => {
	...
	const { mutateAsync: mutateAsyncUpdate } = useMutation({
		mutationFn: async (payload: TForm) => {
			...
			return await kyInstance.patch('groups/update', { json: group }).json<Group>()
		},
	...
	const { mutateAsync: mutateAsyncDelete } = useMutation({
		mutationFn: async () => {
			...
			await kyInstance.delete('groups/delete', { json }).json<Group>()
		},
		onMutate: async () => {
